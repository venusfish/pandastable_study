...
class TableModel(object):
    ...
    def __init__(self, dataframe=None, rows=20, columns=5): # Constructor for table model. """
        self.initialiseFields()
        self.setup(dataframe, rows, columns)
        return

    def setup(self, dataframe, rows=20, columns=5): # Create table model"""
    ...
    @classmethod
    def getIrisData(self): # Get iris dataset"""
    ...
    @classmethod
    def getStackedData(self): # Get a dataframe to pivot test"""
    ...
    def initialiseFields(self): # Create meta data fields"""
        self.meta = {}
        #self.columnwidths = {} #used to store col widths
        return

    def save(self, filename): # Save dataframe"""
    ...
    def load(self, filename, filetype=None): # Load file, if no filetype given assume it's pickle format"""
    ...
    def getlongestEntry(self, colindex, n=500): # Get the longest string in the column for determining width. Just uses the first n rows for speed"""
    ...
    def getRecordAtRow(self, rowindex): # Get the entire record at the specifed row"""

        record = self.df.iloc[rowindex]
        return record

    def moveColumn(self, oldindex, newindex): # Changes the order of columns"""
    ...
    def insertRow(self, row): # Inserts a row at the required index by append/concat"""
    ...
    def deleteRow(self, row, unique=True): # Delete a row"""

        self.deleteRows([row], unique)
        return

    def deleteRows(self, rowlist=None, unique=True): # Delete multiple or all rows"""
    ...
    def addColumn(self, colname=None, dtype=None, data=None): # Add a column"""
    ...
    #     df = df.transpose()
    #     df.reset_index()
    #     if util_study.check_multiindex(df.columns) != 1:
    #         try:
