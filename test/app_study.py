...
from __future__ import absolute_import, print_function # 파이썬 3에서 쓰던 문법을 파이썬 2에서 쓸수있게 해주는 문법

import os, sys, datetime, pickle, gzip
...
try:
    ...
    from tkinter import *
    from tkinter.ttk import *
if (sys.version_info > (3, 0)): # Python version 확인방법
...
from collections import OrderedDict # 실행에 관계없음
import matplotlib
matplotlib.use('TkAgg')
import pandas as pd
import re, platform, time
from core_study import Table

from data_study import TableModel
# from .prefs import Preferences
# from pandastable import images, util, dialogs, plotting, config # 원래의 코드

import images_study, util_study, dialogs_study, plotting_study, config_study
from dialogs_study import MultipleValDialog

from collections import OrderedDict # OrderedDict 때문에 추가함

import plugin_study #주석처리해도 영향이 없음
...
class DataExplore(Frame):
    ...
    def __init__(self, parent=None, data=None, projfile=None, msgpack=None): 
        ...
        if not self.parent:
            Frame.__init__(self)
            self.main=self.master # master와 parent는 비슷한 개념, mastersms object를 돌려주지만, parent는 widget name의 string을 리턴함.
        else:
            self.main=Toplevel() # 별도의 창 컨데티어를 제공하는데 사용됨
            self.master=self.main

        if getattr(sys, 'frozen', False): # Pyinstaller로 만들어진 실행화일을 수행한 경우 임시폴더에 생성되므로 기억해야함
            self.modulepath = os.path.dirname(sys.executable)
        else:
            self.modulepath = os.path.dirname(__file__) # 파이썬으로 실행한 경우의 경로
        ...
        self.currplatform = platform.system() # 운영체제 확인
        self.setConfigDir() # config 파일들이 저장된 directories
        
        #if not hasattr(self,'defaultsavedir'):
        self.defaultsavedir = os.path.join(os.path.expanduser('~')) 
        """ os.path는 파일경로 생성 및 수정하고 파일정보를 쉽게 다룸,  입력 받은
        경로안의 "~"를 현재 사용자의 정대경로로 대체함"""
        
        self.loadAppOptions()
        #start logging
        # self.start_logging()

        self.main.title("EcotoxEndpoint") 
        self.createMenuBar()
        # self.discoverPlugins() #주석처리해도 영향없음
        self.setupGUI()
        self.setStyles()
        ...
        if data != None: # 변수가 Null 인지 확인하는 방법중의 하나
            self.data = data
        ...
        # else:
        #     self.newProject()
        self.main.protocol('WM_DELETE_WINDOW',self.quit) # window closing event
        self.main.lift()
        return
    ...
    def setStyles(self):
        ...
        plf = util_study.checkOS()
        if plf == 'linux':
            style.theme_use('default')
        elif plf == 'darwin':
            style.theme_use('clam')

        self.bg = bg = self.style.lookup('TLabel.label', 'background') # 프로그램 실행에는 관련없음, 나중에 확인
        style.configure('Horizontal.TScale', background=bg)
       
        #set common background style for all widgets because of color issues
        #if plf in ['linux','darwin']:
        #    self.option_add("*background", bg)
        dialogs_study.applyStyle(self.menu)
        return

    def setConfigDir(self): # Set up config folder""", Home directory 확인하는 방법
    ...
    def setupGUI(self): # Add all GUI elements"""

        self.m = PanedWindow(self.main, orient=HORIZONTAL) # PanedWindow은 다른위젯을 수평또는 수직으로 배열하여 창에 숫자를 포함할수 있는 컨테이너용 위젯임.
        self.m.pack(fill=BOTH,expand=1)
        self.nb = Notebook(self.main) # Notebook을 이용하여 페이지를 나눌 수 있는 탭 메뉴를 생성할 수 있음.
        self.m.add(self.nb)
        self.setGeometry()
        return

    def createMenuBar(self): # Create the menu bar for the application. 
        ...
        #필요없는 메뉴, 실제 메뉴에서는 Tool, 없애면 오류발생- 그럼므로 확인 요망
        self.endpoint_menu={'01Point Estimates':{'cmd':self.describe},
                         '02Hypothesis Tests':{'cmd':lambda: self._call('convertColumnNames')},
        ...
        self.endpoint_menu = self.createPulldown(self.menu,self.endpoint_menu)
        self.menu.add_cascade(label='Endpoints',menu=self.endpoint_menu['var'])
        ...
        self.help_menu={'01Online Help':{'cmd':self.online_documentation}, # Help 메뉴관련 내용
    ...
    def createRecentMenu(self, menu): # Recent projects menu"""
    ...
    def loadMeta(self, table, meta): # Load meta data (데이터 속성정보) for a sheet, this includes plot options and table selections"""
        ...
        for m in opts:
            if m in meta and meta[m] is not None:
                ...
                defaults = plotting_study.get_defaults(m)
                for key in defaults:
                    if key not in opts[m].opts:
                        opts[m].opts[key] = defaults[key]

        #load table settings
        util_study.setAttributes(table, tablesettings)
        #load plotviewer
        if 'plotviewer' in meta:
            #print (meta['plotviewer'])
            util_study.setAttributes(table.pf, meta['plotviewer'])
            table.pf.updateWidgets()

        if childtable is not None:
            table.createChildTable(df=childtable)
            util_study.setAttributes(table.child, childsettings)
    ...
    def saveMeta(self, table):
        ...
        meta['table'] = util_study.getAttributes(table)
        meta['plotviewer'] = util_study.getAttributes(table.pf)
        #print (meta['plotviewer'])
        ...
        if table.child != None:
            ...
            meta['childselected'] = util_study.getAttributes(table.child)
    ...
    def editSheetDescription(self):
        ...
        from dialogs_study import SimpleEditor
    ...
    #     #print (pluginpaths)
    #     failed = plugin_study.init_plugin_system(pluginpaths)
    ...
    #     for plg in plugin_study.get_plugins_classes('gui'):
    #         def func(p, **kwargs):
    #             def new():
    #                self.loadPlugin(p)
    #             return new
    ...
    def about(self):
        ...
        abwin = Toplevel()
        x,y,w,h = dialogs_study.getParentGeometry(self.main)
        ...
        logo = images_study.tableapp_logo()
    ...
    def showErrorLog(self): # Open log file"""

        from core_study import logfile
        f=open(logfile,'r')
        s=''.join(f.readlines())
        from dialogs_study import SimpleEditor
        w = Toplevel(self)
    ...
    def online_documentation(self,event=None): # help 메뉴관련 합수
...
class TestApp(Frame): # Basic test frame for the table"""
    def __init__(self, parent=None):
        ...
        options = {'floatprecision': 5, 'textcolor':'blue'}  #options = config.load_options()
        pt.show()
        config_study.apply_options(options, pt)
        self.table.rowheader.bgcolor = 'orange'
        self.table.colheader.bgcolor = 'lightgreen'
        self.table.colheader.textcolor = 'purple'
       
        pt.setRowColors(rows=range(2,100,2), clr='lightblue', cols='all')  #test row coloring
       
        pt.setSelectedRows([[4,6,8,10]])  #test deleting
        pt.deleteRow()
        pt.setSelectedRow()
        pt.insertRow()
        pt.redraw()
