...
import util_study, images_study
...
class CombineDialog(Frame):
    ...
    def getResult(self, df):
        ...
        f = self.tbf = Frame(self.main)
        f.pack(side=LEFT,fill=BOTH)
        newtable = core_study.Table(f, dataframe=df, showstatusbar=1)
...
class QueryDialog(Frame):
    ...
    def setup(self):
        ...
        f = Frame(qf)
        f.pack(side=TOP, fill=BOTH, padx=2, pady=2)
        addButton(f, 'find', self.query, images_study.filtering(), 'apply filters', side=LEFT)
        addButton(f, 'add manual filter', self.addFilter, images.add(),
                  'add manual filter', side=LEFT)
        addButton(f, 'close', self.close, images_study.cross(), 'close', side=LEFT)
        ...
        addButton(f, 'color rows', self.colorResult, images_study.color_swatch(), 'color filtered rows', side=LEFT)
