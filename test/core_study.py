...
from headers_study import ColumnHeader, RowHeader, IndexHeader
from plotting_study import MPLBaseOptions, PlotViewer


#from .prefs import Preferences
# from pandastable.dialogs import ImportDialog
# from pandastable import images, util, config
# from pandastable.dialogs import *
from dialogs_study import ImportDialog
import images_study, util_study, config_study
from dialogs_study import *
...
config_path = os.path.join(os.path.expanduser("~"), '.testing')
...
class Table(Canvas):
    ...
    def close(self, evt=None):
        if hasattr(self, 'parenttable'): # 변수가 있는지 확인
            return
        if hasattr(self, 'pf') and self.pf is not None:
            #print (self.pf)
            self.pf.close()
        if util_study.SCRATCH is not None:
            util_study.SCRATCH.destroy()
            util_study.SCRATCH = None
        return

    def set_defaults(self): # Set default settings"""
    ...
    def setFont(self): # Set font tuple"""
    ...
    def setTheme(self, name='light'): # Set theme"""
    ...
    def mouse_wheel(self, event): # Handle mouse wheel scroll for windows"""
    ...
    def doBindings(self): # Bind keys and mouse clicks, this can be overriden"""
    ...
    def resized(self, event): # Check if size changed when event triggered to avoid unnecessary redraws"""
    ...
    def adjustColumnWidths(self, limit=30):
        ...
        for col in range(self.cols):
            ...
            tw,tl = util_study.getTextLength(txt, self.maxcellwidth,
                                       font=self.thefont)
            #print (col,txt,l,tw)
    ...
    def evalBar(self, evt=None):
        ...
        addButton(bf, 'apply', self.evalFunction, images_study.accept(), 'apply', side=LEFT)
        addButton(bf, 'preset', self.applyColumnFunction, images_study.function(), 'preset function', side=LEFT)
        addButton(bf, 'clear', clear, images_study.delete(), 'clear stored functions', side=LEFT)
        addButton(bf, 'close', reset, images_study.cross(), 'close', side=LEFT)
    ...
    def crosstab(self):
        ...
        df = self.model.df
        from dialogs_study import CrosstabDialog
    ...
    def doCombine(self):
        ...
        if self.child == None:
            ...
            return
        self.storeCurrent()
        from dialogs_study import CombineDialog
    ...
    def drawText(self, row, col, celltxt, align=None, fgcolor='black'):
        ...
        tw,newlength = util_study.getTextLength(celltxt, w-pad, font=self.thefont)
        width=0
    ...
    def drawMultipleRows(self, rowlist):
        ...
        for col in cols:
            ...
            if colname in self.columncolors:
                clr = self.columncolors[colname]
                clr = util_study.colorScale(clr, -30)
            else:
    ...
    def showPreferences(self):
        ...
        options = config_study.load_options()
        f = config_study.preferencesDialog(self, options, table=self)
        return
    ...
    def loadPrefs(self, prefs=None):
        ...
        options = config_study.load_options()
        config_study.apply_options(options, self)
        return
