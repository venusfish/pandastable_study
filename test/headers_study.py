...
class ColumnHeader(Canvas):
    ...
    def __init__(self, parent=None, table=None, bg='gray25'):
        ...
        if table != None:
            ...
            if util_study.check_multiindex(self.model.df.columns) == 1:
                self.height = 40
            else:
    ...
    def redraw(self, align='w'):
        ...
        df = self.model.df
        multiindex = util_study.check_multiindex(df.columns)
        wrap = self.wrap
        ...
        if wrap is True:
            ...
            tw,l = util_study.getTextLength(longest, cw)
        ...
        for level in levels:
            ...
            for col in self.table.visiblecols:
                ...
                colname = colstr
                tw,length = util_study.getTextLength(colstr, w-pad, font=font)
                #print (colstr, tw)
                if wrap is True:
    ...
    def popupMenu(self, event):
        ...
        if len(df.columns)==0:
            return
        ismulti = util_study.check_multiindex(df.columns)
...
class RowHeader(Canvas):
    ...
    def redraw(self, align='w', showkeys=False):
        ...
        if self.table.showindex == True:
            if util_study.check_multiindex(index) == 1:
                ind = index.values[v]
